...
Using this script with Frida was crashing the app and sometimes caused the device to reboot. Also, the Frida script is not compatible with Frida 17.x, so I decided to create a patching script that modifies `libflutter.so` directly. This way, you can patch the file _once_ and use it on any device without needing to run a Frida script.

> [!Note]
> Patching `libflutter.so` for TLS verification will not make the app proxy-aware. It will only disable TLS verification, allowing you to intercept the traffic. You may still need to tunnel/force the app's traffic through other means (e.g., using a VPN or modifying the app's network configuration) to ensure that the traffic is routed through your proxy tool.
